...
- Quarto, by default, creates a final section in your document named 'References' with a formatted reference list.
...
- cite one of the articles using the `@`
...
Code is executed when the document is rendered. Code blockss can also be executed individually just like regular R markdown if you are already familiar with that. Code blocks are inserted like this:
```{r}
#| echo: fenced
#| eval: false
6*7
...
Code block options are set with a special comment: `#| ` (white space sensitive!). [Block options](https://quarto.org/docs/computations/execution-options.html) determine how the code is executed, e.g., whether to display the code block: `#| echo: true`, warning messages `#| warning: false`, and to input captions `#| fig-cap: "A really good figure caption!"`.
...
- Check out the [Block options](https://quarto.org/docs/computations/execution-options.html) and edit the existing code block options to:
...
Ok now the feature I find *very* useful: tabsets. Tabsets are great for showing, multiple results, data, code, whatever you want in tabs.
